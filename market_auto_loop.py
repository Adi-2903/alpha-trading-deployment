    ...
    while True:
        try:
            ...
            if elapsed < args.interval and last_run_ts > 0:
                ...
                print(f"{GREEN}[{_now()}] OPEN - closes {ncf} - cycle #{cycle_count} done - next in {mins:02d}:{secs:02d} - ${equity} - {mode_label}{RESET}", end="\r")
                time.sleep(min(POLL_SECONDS, time_to_next))
                continue

            cycle_count += 1
            print(f"\n{BOLD}{GREEN}[{_now()}] >> CYCLE #{cycle_count}  (market open - closes {ncf} - equity ${equity}){RESET}")
            ...
            if result:
                ...
                print(f"  {DIM}Next cycle in {args.interval//60} min | Ctrl+C to stop{RESET}")
                print()
            else:
